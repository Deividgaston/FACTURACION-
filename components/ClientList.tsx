...
import { Mail, MapPin, UserPlus, Search, Trash2 } from 'lucide-react';
import { store } from '../lib/store';
import { Party } from '../types';

const ClientList: React.FC = () => {
  const [clients, setClients] = useState<any[]>(store.getClients());
  ...
  const handleDelete = (id: string) => {
    ...
    localStorage.setItem('si_clients', JSON.stringify(updated));
    setClients(updated);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={() => setShowAdd(true)} className="bg-indigo-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-indigo-700 transition-all flex items-center gap-2">
